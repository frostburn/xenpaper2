...
import { getNoiseGeneratorProcessorName } from './noise-worklet'
...
/**
 * White noise AudioWorkletNode in series with a GainNode.
 */
export class EnvelopedNoiseGenerator implements GainNode {
  private noiseNode: AudioWorkletNode
  private gainNode: GainNode

  constructor(context: BaseAudioContext) {
    this.noiseNode = new AudioWorkletNode(context, getNoiseGeneratorProcessorName(), {
      numberOfInputs: 0,
      numberOfOutputs: 1,
  ...
  get detune() {
    return this.noiseNode.parameters.get('detune')!
  }

  get frequency() {
    return this.noiseNode.parameters.get('frequency')!
  }
  ...
  set type(_value: OscillatorType) {}
