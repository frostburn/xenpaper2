...
import { Bank, OfflineBank } from '../sw-seq/bank'
...
export const useXenpaperStore = defineStore('xenpaper', () => {
  ...
  const renderSongToWavBlob = async (tailSeconds: number): Promise<Blob> => {
    ...
    const renderDuration = frameCount / audioContext.sampleRate
    ...
    const bank = new OfflineBank(offlineContext)
    ...
    try {
      transport.endTime = renderDuration
      transport.start(0)
